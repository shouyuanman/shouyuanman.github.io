...
**业务需求**：在单库单表场景下，生成并使用**全局唯一**`ID`（比如可用作短链`ID`、订单`ID`、交易单`ID`、消息流水`ID`、调查问卷`ID`、用户`ID`、聊天会话`ID`等等），要求`ID`**业务可读**，**`Long`整型且无规律**（隐藏自增特性），‌能**防爬取/猜测**。

## **思路**
服务自己闭环，将数据库自增主键转换为短且无规律的公开`ID`，用算法隐藏业务数据的递增特性，防止被遍历或猜测。一般业务服务都会用到`MySQL`数据库，生成`ID`时也用`MySQL`数据库，这样就意味着没有引入更多的组件依赖。
