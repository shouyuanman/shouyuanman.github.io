...
3. 跨单元数据同步组件
    ...
    每个单元平时都要承担业务流量，处于“活”的状态。同时，每个单元要能按照百分比承担业务流量，并且能够动态的进行调整。比方说，不同`IDC`机房，`MySQL`之间的数据异步复制，数据库如何同步？应用的双活和中间件的双活，最终都依赖于数据如何存放。如果两个机房中各部署一个数据库，那么机房间的数据如何同步呢？
